...
import {Class,Student,SortData} from './type'
 ...
 * @param flags option.
 */
const main = (args: string[], flags: meow.TypedFlags<any> & { [p: string]: unknown }) : void => {
    let data:SortData
    ...
    displaySortingResult(sorting(data))
}
...
const sorting = (data: SortData): Class[] => {
    ...
    const classStudentNum = {
        max: Math.round(data.students.length / data.classNames.length),
    ...
    students.forEach((student) => {
        while (true) {
            const classRandomIndex =  Math.round(Math.random() * (classes.length - 1))

            if (classes[classRandomIndex].students.length < classStudentNum.min) {
 ...
 *  @param classes Class
 */
const displaySortingResult =  (classes: Class[])  => {
    console.log('Result !!')
    console.log('========================')
    classes.forEach(c => {
        console.log(c.name)
        console.log('----------------------')
        c.students.forEach(value => {
